...
SECRET_KEY = "mysecretkey"  # тут мы используем что-нибудь вроде команды Bash (Linux) 'openssl rand -hex 32'
ALGORITHM = "HS256"  # плюс в реальной жизни мы устанавливаем "время жизни" токена


USERS_DATA = [{"username": "admin", "password": "adminpass"}]
# в реальной БД мы храним только ХЭШИ паролей  (библиотека passlib) + соль (добавка к паролю)
...
# роут для аутентификации (не очень правильно, но пока так) - получаем токен
@app.post("/login")
...
# защищенный роут для получения информации о пользователе (Bearer Token)
@app.get("/about_me")
